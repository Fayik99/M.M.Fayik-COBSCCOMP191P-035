//
//  CovidSurveyViewController.swift
...
class CvSurveyViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
//        let que6 = Question( questionText: "What is the color of sky?", options: ["Violet", "Yellow", "Blue", "White"], correctAns: 2, wrongAns: -1, isAnswered: false)
    ...
    let btnPrev: UIButton = {
     ...
     //   btn.addTarget(self, action: #selector(btnPrevNextAction), for: .touchUpInside)
        btn.addTarget(self, action: #selector(btnPrevNextAction), for: UIControl.Event.touchUpInside)
        return btn
    }()
